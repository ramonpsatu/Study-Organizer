...
class FridayListAdapter (
    private val itemClick: (Int) -> Unit,
    private val viewModel: SubjectListViewModel
) : RecyclerView.Adapter<FridayListAdapter.ViewHolder>() {
    ...
    fun updateSubjectFriday(fridayList: List<SubjectItem>) {
    ...
    inner class ViewHolder(
        ...
        fun bind(subject: SubjectItem) {
            ...
            binding.textViewPerformanceNumber.text = itemView.context.getString(
                com.ramonpsatu.studyorganizer.core.ui.R.string.text_performance_number
            )

            viewModel.viewModelScope.launch {
                //Defines the text number of topics
                val date = android.text.format.DateFormat.format("yyyyMMdd",viewModel.stateOnceDates().value!!.datesOfWeek[4]).toString()
